...
function hitungStatistik(arrMahasiswa) {
    ...
    const nilaiTertinggi=arrMahasiswa.reduce((max, mhs)=>mhs.nilai>max ? mhs.nilai:max,
arrMahasiswa[0].nilai);
    const nilaiTerendah=arrMahasiswa.reduce((min, mhs)=>mhs.nilai<min ? mhs.nilai:min,
arrMahasiswa[0].nilai);
